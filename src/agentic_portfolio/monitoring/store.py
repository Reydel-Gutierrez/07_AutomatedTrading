"""Persist position-monitoring runs. Never overwrite history."""
...
def monitoring_dir(root: Path | None = None) -> Path:
    return (root or project_root()) / "state" / "position_monitoring"


class MonitoringStore:
    def __init__(self, root: Path | None = None) -> None:
        self.root = monitoring_dir(root)
    ...
    def save(self, run_id: str, record: dict[str, Any]) -> Path:
        ...
        if path.exists():
            raise FileExistsError(f"monitoring run already exists: {run_id}")
